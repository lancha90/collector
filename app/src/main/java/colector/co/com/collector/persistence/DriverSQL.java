...
//			super(context, "/storage/sdcard0/DB_DMS_MOBILE.db", factory, AppSettings.DB_VERSION);
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("create table if not exists TBL_RESPONSE_COMPLEX ( ID  TEXT , QUESTION  INTEGER , FORM TEXT,PRIMARY KEY (ID));");
			...
			db.execSQL("CREATE TABLE IF NOT EXISTS TBL_SURVEY_INSTANCE ( ID  INTEGER PRIMARY KEY AUTOINCREMENT, ID_SURVEY  INTEGER, DATE_INIT  TEXT,DATE_END  TEXT,LATITUDE  TEXT,LONGITUDE  TEXT,STATUS TEXT NOT NULL DEFAULT 'FALSE');");
			db.execSQL("CREATE TABLE IF NOT EXISTS TBL_SURVEY_INSTANCE_DETAIL ( ID  INTEGER PRIMARY KEY AUTOINCREMENT, ID_INSTANCE  INTEGER,ID_QUESTION  INTEGER, ANSWER TEXT, ANSWER_OV TEXT);");
